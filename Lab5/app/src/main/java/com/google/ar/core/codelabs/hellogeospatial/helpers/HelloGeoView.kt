...
//        mapView?.googleMap?.projection?.fromScreenLocation(screenLocation) ?: return@setup
        val pair: Pair<LatLng, Float> =
            Pair(mapView?.googleMap?.projection?.fromScreenLocation(screenLocation.first) ?: return@setup, screenLocation.second)
      activity.renderer.onMapClick(pair.first, pair.second)
    }
  }
